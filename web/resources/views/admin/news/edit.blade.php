...
@section('title', 'Editer le cours : ' . $course->title)

@section('content')
	<div class="container-fluid">
		<div class="row">
            <div class="col-12">
                <h1>Editer le cours {{ $course->title }}</h1>
	                ...
	                <div class="card mb-4">
	                    <div class="card-body">
	                    	...
	                    	<div class="form-group">
                                ...
                                <input type="text" class="form-control input-small @error('title') is-invalid @enderror" name="title" id="title" value="{{ old('title', $course->title) }}" placeholder="Titre">
                            	@error('title')
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="duration" class="form-control" value="{{ old('duration', $course->duration) }}" placeholder="Durée estimé">
                                @error('duration')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" disabled class="form-control" value="{{ $course->module->classe->scholarYear->scholar_year }}">
                            </div>
	                    </div>
	                </div>
		    	</div>
		    		...
		    		<div class="card mb-4">
	                    <div class="card-body">
		                            ...
		                            <div class="form-group">
		                                ...
		                                <select name="level_id" class="form-control select2 @error('level_id') is-invalid @enderror" id="level_id">
		                                	...
		                                	@foreach ($levels as $level)
		                                		<option {{ old('level_id', $course->level_id) == $level->id ? 'selected' : '' }} value="{{ $level->id }}">{{ $level->title }}</option>
		                                	@endforeach
		                                </select>
		                            	@error('level_id')
		                                    <span class="invalid-feedback" role="alert">
                            ...
                            <div class="row">
		                            ...
		                            <div class="form-group">
		                                ...
		                                <select name="teacher_id" class="form-control select2 @error('teacher_id') is-invalid @enderror" id="teacher_id">
		                                	@foreach ($teachers as $teacher)
		                                		<option {{ old('teacher_id', $course->teacher_id) == $teacher->id ? 'selected' : '' }} value="{{ $teacher->id }}">{{ $teacher->user->full_name }}</option>
		                                	@endforeach
		                                </select>
		                            	@error('teacher_id')
		                                    <span class="invalid-feedback" role="alert">
                            	...
                            	<div class="col-md-6">
                            		<div class="form-group">
		                                ...
		                                <input type="text" class="form-control datepicker @error('published_at') is-invalid @enderror" name="published_at" readonly autocomplete="off" id="published_at" value="{{ old('published_at', Helper::formatDate($course->published_at)) }}" placeholder="Date de publication">
		                            	@error('publushd')
		                                    <span class="invalid-feedback" role="alert">
		                                        <strong>{{ $message }}</strong>
		                                    </span>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="video_content" class="form-control @error('video_content') is-invalid @enderror" cols="30" rows="10" placeholder="Live source">{{ old('video_content', $course->video_content) }}</textarea>
                            	@error('video_content')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
