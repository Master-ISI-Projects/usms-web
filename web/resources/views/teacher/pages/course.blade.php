        ...
        <div class="col-12">
            <div class="mb-2">
                ...
                <div class="top-right-button-container">
                    <h3>
                        <span class="badge badge-pill badge-outline-danger mb-1">{{ $course->module->title }} / {{ $course->module->classe->title }}</span>
                    </h3>
                </div>
            </div>
                        ...
                        @forelse ($course->files as $file)
                            <div class="d-flex flex-row pb-3 justify-content-between {!! $loop->last ?: 'border-bottom mb-3' !!}">
                                ...
                                <div class="comment-likes">
                                    <form method="post" action="{{ route('teacher.files.delete', ['id' => $file->id]) }}">
                                        @csrf
                                        @method('DELETE')
...
<div class="modal" id="modal-add-document" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form method="post" action="{{ route('teacher.courses.save_document', ['id' => $course->id]) }}" enctype="multipart/form-data">
                @csrf
